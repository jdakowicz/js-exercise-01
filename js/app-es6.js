...
//     addClass(clickedEl);
//     tag === 'all' ? toggleAll(true) : filter(tag);
//   };
...
//   function checkItem (tag, tagList) {
//     return tagList.some(item => item === tag.replace(' ', '-'));
//   };

//   function filter (activeTag) {
//     $elements.items.map(item => {
//       const tagsText = item.querySelector('figcaption').innerText;
//       const tagsList = tagsText.replace(/[#,]/g, '').split(' ');

//       toggleItem(item, checkItem(activeTag, tagsList));
//     });
//   };
